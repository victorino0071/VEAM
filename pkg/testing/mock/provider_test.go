...
func TestMockProvider_ResolutionHierarchy(t *testing.T) {
	...
	mockProvider := mock.NewMockProvider(1.0, 0)
	
	// L2: Regra que aprova transações "VIP"
	mockProvider.AddRule(func(ctx context.Context, tx *entity.Transaction) *mock.MockResponse {
		if tx.CustomerID == "customer_vip" {
			return &mock.MockResponse{ExternalID: "ext_vip_ok", Err: nil}
		}
		return nil
	})
	...
	t.Run("L1 (Override) deve vencer L2", func(t *testing.T) {
		...
		mockProvider.RegisterOverride("tx_override", &mock.MockResponse{
...
func TestMockProvider_ConcurrencySafe(t *testing.T) {
	mockProvider := mock.NewMockProvider(0.1, 10*time.Millisecond)
	
	t.Run("Parallel access to overrides", func(t *testing.T) {
		t.Parallel()
		for i := 0; i < 100; i++ {
			go func(val int) {
				mockProvider.RegisterOverride("tx_id", &mock.MockResponse{ExternalID: "ok"})
