...
#### `ProcessPaymentWithMetadata(ctx, transactionID, metadata, nextStatus)`
...
**A orquestração perfeita se comporta desse jeito minuncioso:**
1.  **Entrada transacional:** Como essa modificação na transação criará _side effects_ e mexerá fisicamente nos bytes gravados, a premissa base dele é abrir primeiramente `ExecuteInTransaction`. Uma promessa ACID (do BD) é repassada pelo Service via _closures_.
2.  **Consulta (`GetTransactionByID`)**: Estando na transação estrita, busca fisicamente a "Transaction" pelo ID usando a regra da Porta.
